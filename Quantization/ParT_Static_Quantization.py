...
class Block(nn.Module):
    ...
    def forward(self, x, x_cls=None, padding_mask=None, attn_mask=None):
        ...
        if self.c_attn is not None:
            ...
            #x = torch.einsum('tbhd,h->tbdh', x, self.c_attn)         # me 
    ...
    def forward(self, x, v=None, mask=None, uu=None, uu_idx=None):
        ...
        with torch.cuda.amp.autocast(enabled=self.use_amp):
            ...
            for block in self.blocks:
                #x = self.quant(x)                       # me added this line
                custom_module_config = {"float_to_observed_custom_module_class": {torch.nn.MultiheadAttention: torch.nn.quantizable.MultiheadAttention},
                                        "observed_to_quantized_custom_module_class": {torch.nn.quantizable.MultiheadAttention: torch.nn.quantized.MultiheadAttention},
                                        }
                block.qconfig = torch.ao.quantization.get_default_qconfig('qnnpack')
                model_prepared = torch.ao.quantization.prepare(block, prepare_custom_config_dict=custom_module_config)
                
                model_prepared(x, x_cls=None, padding_mask=padding_mask, attn_mask=attn_mask)     # should be used something smaller than the complete inference example_inputs
                model_int8 = torch.ao.quantization.convert(model_prepared,convert_custom_config_dict=custom_module_config,)
                
                print('fp32 model Size (MB):', print_size_of_model(block,'model'))
                print('model_int8 Size (MB):', print_size_of_model(model_int8, 'model_int8'))
                
                x = model_int8(x, x_cls=None, padding_mask=padding_mask, attn_mask=attn_mask)
                self.interactionMatrix = attn_mask
                self.attention_matrix = block.interaction 
                #x = self.dequant(x)                     # me added this line
